...
export const loggerPlugin = new Elysia()
  .onRequest(({ request }) => {
    ...
    const hasToken =
      parseCookie(request.headers.get("cookie") ?? "")?.token !== undefined;
    console.log(
  ...
  .onError(({ request, error }) => {
    console.log(
      `❌ ${request.method} ${request.url} ${error.message.substring(0, 100)}`
    );
    console.error(error);
  })
  .onResponse(({ path, request, set }) => {
    ...
    console.log(
      `${
        typeof set.status === "number" ? emojiStatus.get(set.status) ?? "" : ""
